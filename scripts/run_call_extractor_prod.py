...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run production call extractor inference + production decoder.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If set, default output prefix becomes call_extractor/wavlm_large_v1/models/<run_id>/decoded_prod_<ts>/",
    )
    parser.add_argument(
        "--out-s3-prefix",
        type=str,
        default=None,
        help="Explicit decode-scoped output prefix (overrides --run-id behavior).",
    )
    parser.add_argument(
        "--subset",
        type=str,
    ...
    if out_s3_prefix is None:
        if not args.run_id:
            raise ValueError("Must pass --out-s3-prefix or --run-id to avoid overwriting root segments/")
        out_s3_prefix = f"call_extractor/wavlm_large_v1/models/{args.run_id}/decoded_prod_{_utc_ts()}"
    ...
    logger.info("out_s3_prefix=%s videos=%d subset=%s", out_s3_prefix, len(video_ids), args.subset)

    for idx, vid in enumerate([str(v) for v in video_ids], start=1):
        audio_key = audio_index.get(str(vid))
        if not audio_key:
            logger.warning("[%d/%d] Skip %s: audio not found", idx, len(video_ids), vid)
            continue
        ...
        segments = decode_production_segments(times_s=t, in_call_p=in_call, start_p=start, end_p=end, cfg=decode_cfg)

        out_json = {
            "video_id": str(vid),
            "audio_s3_key": str(audio_key),
            "model_dir": str(args.model_dir),
            "out_s3_prefix": str(out_s3_prefix),
            "segments": segments,
        ...
        if args.upload_segments:
            dst_key = f"{out_s3_prefix}/segments/{vid}.json"
        ...
        if args.write_probs or args.upload_probs:
            ...
            if args.upload_probs:
                dst_key = f"{out_s3_prefix}/probs/{vid}.npz"
                s3_upload_file(bucket=S3_BUCKET, key=dst_key, src_path=npz_path, region=AWS_REGION)

        logger.info("[%d/%d] %s: segments=%d frames=%d", idx, len(video_ids), vid, len(segments), int(t.shape[0]))
