...
Usage:
    python scripts/eval_gemini_call_detector.py \
        --run-dir artifacts/gemini_pilot/eval_batch_XXXXXXXX \
...
def _detect_and_parse_timestamps(segments_raw: list[dict]) -> list[Segment]:
    ...
    """
    # Collect all raw timestamps for format detection
    all_ts: list[str] = []
    for seg in segments_raw:
        all_ts.append(seg.get("start", ""))
        all_ts.append(seg.get("end", ""))

    # Detect: do all timestamps have a third field, and is it always "00"?
    ...
    if three_field_count > 0 and two_field_count == 0:
        # All three-field — check if third is always "00"
        all_third_zero = all(f.strip() == "00" for f in third_fields)
        if all_third_zero:
            fmt = "MMSS00"  # MM:SS:00 → parse first two as minutes:seconds
        else:
            fmt = "HHMMSS"  # True HH:MM:SS
    else:
        fmt = "MMSS"  # Two-field or mixed (treat as MM:SS)
...
def _ts_to_seconds(ts: str, fmt: str) -> float | None:
    ...
    elif fmt == "HHMMSS" and len(nums) == 3:
...
def match_segments(
    ...
    """Greedy IoU matching: best pairs first, each segment used at most once."""
    ...
    for score, gi, pi in candidates:
        ...
        results.append(MatchResult(
            label=label, gt=g, pred=p, iou=score,
    ...
    # Unmatched predictions → EXTRA
    for pi, p in enumerate(pred):
        if pi not in matched_pred:
            results.append(MatchResult(label="EXTRA", pred=p))

    # Sort results by time for readable output
...
def _download_gt_files(s3_prefix: str, video_ids: list[str], local_dir: Path) -> list[Path]:
    """Download only the required GT JSON files from S3 using AWS CLI.

    This is intentionally *not* `aws s3 sync` on the whole prefix, which can be
    extremely slow if the label prefix contains many files.
    """
...
def load_ground_truth(
    ...
    *,
    ...
    """Load ground truth boundaries from S3 v1 label files.

    Returns: {video_id: [Segment, ...]}
    """
    ...
    for f in files:
        try:
            ...
            logger.warning(f"Skipping malformed GT file: {f}")
            continue
...
# Source ID patterns: "local_{stem}_{hash}" or "s3_{stem}_{hash}"
# The stem may contain the video ID as the last segment after " - " or just be the video ID
_VIDEO_ID_FROM_STEM = re.compile(r"^(?:local|s3)_(.+?)_[0-9a-f]{6,12}$")


def _extract_video_id(source_id: str) -> str | None:
    """Extract video ID from a source_id like 'local_DEt3IRqqUVs_0971ec26' or
    's3_10_Live_Cold_Calls_..._-_DEt3IRqqUVs_abcd1234'.
    """
    m = _VIDEO_ID_FROM_STEM.match(source_id)
    if not m:
        return None
    stem = m.group(1)
    # If stem contains " - ", the video ID is after the last separator
    # The sanitize_token replaces spaces with _, so look for _-_
    if "_-_" in stem:
...
def _recover_truncated_segments(path: Path) -> dict | None:
    """Try to recover segments from a truncated JSON file.

    Strategy: extract all complete {"start": "...", "end": "..."} objects via regex.
    """
    text = path.read_text(errors="replace")
    pattern = re.compile(
        r'\{\s*"start"\s*:\s*"([^"]+)"\s*,\s*"end"\s*:\s*"([^"]+)"\s*\}'
    )
...
def load_predictions(run_dir: Path) -> dict[str, dict[str, list[Segment]]]:
    """Load Gemini predictions from a run directory.

    Returns: {video_id: {model: [Segment, ...]}}
    """
    ...
    for result in summary.get("results", []):
        ...
        if not video_id:
            logger.warning(f"Cannot extract video_id from source_id={source_id}")
            continue
        ...
        try:
            ...
            # Attempt recovery of truncated JSON (Gemini sometimes cuts off mid-stream)
            raw_data = _recover_truncated_segments(raw_path)
            if raw_data is None:
                logger.warning(f"Malformed JSON (unrecoverable) in {raw_path}")
                continue
            logger.info(
                f"Recovered {len(raw_data.get('segments', []))} segments "
                f"from truncated output in {raw_path}"
            )
...
# ---------------------------------------------------------------------------
# Display: per-video timeline
# ---------------------------------------------------------------------------

def print_video_eval(ve: VideoEval) -> None:
    """Print a diagnostic timeline for one video × model."""
    console.print(f"\n[bold]═══ {ve.video_id} — {ve.model} ═══[/bold]")
    console.print(
        f"GT: {len(ve.gt_segments)} segments │ "
        f"Predicted: {len(ve.pred_segments)} segments"
    )
    console.print()
    ...
    for r in ve.results:
        if r.label in ("MATCH", "BOUNDARY_OFF"):
            ...
            if extra_shown <= max_extra_shown:
                console.print(
                    f"  [magenta]{'EXTRA':14s}[/magenta] "
                    f"{'':28s}Pred {_fmt_range(r.pred)}"
                )
            elif extra_shown == max_extra_shown + 1:
                console.print(
                    f"  [magenta]  ... and {extra_total - max_extra_shown} more EXTRA segments[/magenta]"
                )


# ---------------------------------------------------------------------------
# Display: per-model summary table
# ---------------------------------------------------------------------------

def print_summary_table(summaries: list[ModelSummary]) -> None:
    """Print a rich table summarizing per-model performance."""
...
def _build_json_report(
    ...
    """Build the full JSON report for persistence."""
...
def _build_markdown_report(
    ...
    *,
    ...
    """Build a human-readable Markdown evaluation report."""
    lines: list[str] = []
    w = lines.append  # shorthand

    run_id = args.run_dir.name
    w(f"# Gemini Call Detector — Eval Report")
    w("")
    w(f"**Run:** `{run_id}`  ")
    ...
    w(f"| {_LABEL_ICON['BOUNDARY_OFF']} | BOUNDARY_OFF | Matched, but start or end off by >{int(args.boundary_off_threshold)}s |")
    ...
    # ── Per-video × model detail ──
    # Group evals by video
    ...
    for vid, evals in by_video.items():
        ...
        for ve in evals:
            ...
            w(f"### {ve.model}")
            w(f"GT: **{len(ve.gt_segments)}** segments \u00b7 "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate Gemini call-detector predictions against ground truth."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Minimum IoU to consider a match (default: 0.3)",
    )
    parser.add_argument(
        "--boundary-off-threshold", type=float, default=10.0,
        help="Seconds offset to flag BOUNDARY_OFF (default: 10)",
    )
    parser.add_argument(
        "--out-json", type=Path, default=None,
        help="Path to write JSON report. Default: <run_dir>/eval_report.json",
    )
    parser.add_argument(
        "--gt-cache-dir", type=Path, default=None,
    ...
    for vid in common_ids:
        ...
        for model, pred_segs in sorted(preds[vid].items()):
            ...
            ve = VideoEval(
                video_id=vid, model=model,
                gt_segments=gt_segs, pred_segments=pred_segs,
                results=results,
            )
            for r in results:
                if r.label == "MATCH":
                    ...
                    ve.boundary_off += 1
                    ve.matches += 1  # Counts as a match for F1, but flagged
                elif r.label == "MISSED":
                    ve.missed += 1
                elif r.label == "EXTRA":
                    ve.extra += 1
    ...
    # 5b. Optional details printing (kept off by default for speed/UX)
    ...
    # 7. Markdown report (alongside JSON)
    out_md = out_json.with_suffix(".md")
    md_content = _build_markdown_report(
        all_evals, summaries, args,
